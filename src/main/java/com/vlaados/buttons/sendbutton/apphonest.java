...
public class apphonest implements ButtonHandler {

    private final TelegramBot bot;
    private final ApiClient apiClient = new ApiClient();

    public apphonest(TelegramBot bot) {
        this.bot = bot;
    }

    @Override
    public boolean canHandle(String buttonData) {
        return "apphonest".equalsIgnoreCase(buttonData);
    }
    ...
    private boolean sendRequest(String chatId) {
        String jsonBody = String.format("{\"telegramId\": \"%s\", \"type\": 3}", chatId);
