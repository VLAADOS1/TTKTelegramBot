...
public class appmen implements ButtonHandler {

    private final TelegramBot bot;
    private final ApiClient apiClient = new ApiClient();

    public appmen(TelegramBot bot) {
        this.bot = bot;
    }

    @Override
    public boolean canHandle(String buttonData) {
        return "appmen".equalsIgnoreCase(buttonData);
    }
    ...
    private boolean sendRequest(String chatId) {
        String jsonBody = String.format("{\"telegramId\": \"%s\", \"type\": 6}", chatId);
