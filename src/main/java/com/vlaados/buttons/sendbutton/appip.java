...
public class appip implements ButtonHandler {

    private final TelegramBot bot;
    private final ApiClient apiClient = new ApiClient();

    public appip(TelegramBot bot) {
        this.bot = bot;
    }

    @Override
    public boolean canHandle(String buttonData) {
        return "appip".equalsIgnoreCase(buttonData);
    }
    ...
    private boolean sendRequest(String chatId) {
        String jsonBody = String.format("{\"telegramId\": \"%s\", \"type\": 5}", chatId);
